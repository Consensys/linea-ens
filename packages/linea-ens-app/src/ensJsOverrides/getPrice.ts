import { BaseError, decodeFunctionResult, encodeFunctionData, type Hex } from 'viem'
...
import { SimpleTransactionRequest } from '@ensdomains/ensjs/dist/types/types'
import { GeneratedFunction } from '@ensdomains/ensjs/dist/types/utils/generateFunction'
...
const encode = (
  ...
  const names = (Array.isArray(nameOrNames) ? nameOrNames : [nameOrNames]).map((name) => {
    ...
    if (nameType !== 'eth-2ld' && nameType !== 'eth-3ld' && nameType !== 'tld')
      throw new UnsupportedNameTypeError({
        nameType,
        supportedNameTypes: ['eth-2ld', 'tld', 'eth-3ld'],
        details: 'Currently only the price of eth-2ld and eth-3ld names can be fetched',
      })
    return labels[0]
  })
