...
import { AlertSVG, CrossSVG, EthSVG, Modal, mq } from '@ensdomains/thorin'
import { WithAlert } from '@ensdomains/thorin/dist/types/types'
...
const Icon = ({ alert }: { alert: NonNullableAlert }) => {
  ...
  return <IconContainer $alert={alert}>{isAlertIcon ? <AlertSVG /> : <EthSVG />}</IconContainer>
}
...
const Heading = ({ title, subtitle, alert }: TitleProps & StepProps & WithAlert) => {
  return (
    <TitleContainer>
      ...
      {title &&
        ((typeof title !== 'string' && title) || <Title fontVariant="headingFour">{title}</Title>)}
...
const ModalWithTitle = ({
  ...
  return (
    <Modal {...{ ...props, open, onDismiss }}>
      <StyledCard>
        <Heading {...{ alert, title, subtitle, currentStep, stepCount, stepStatus }} />
        {children}
      </StyledCard>
    </Modal>
  )
