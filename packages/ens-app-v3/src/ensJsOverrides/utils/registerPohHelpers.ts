...
import { CampaignReferenceTooLargeError } from '@ensdomains/ensjs'
import {
  EMPTY_ADDRESS,
...
const cryptoRef =
  ...
  (typeof window !== 'undefined' && typeof window.crypto !== 'undefined' && window.crypto) ||
  undefined
...
export const randomSecret = ({
  ...
  if (campaign) {
    if (campaign > 0xffffffff) throw new CampaignReferenceTooLargeError({ campaign })
...
export const makeRegistrationTuple = (params: RegistrationParameters): RegistrationTuple => {
  ...
  return [label, ...commitmentData, params.signature || '0x']
}
