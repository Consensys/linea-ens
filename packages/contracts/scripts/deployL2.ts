...
async function main() {
  ...
  // Test with subdomain with default "test.lineatest.eth", assuming we still control lineatest.eth on L1
  const name = process.env.L2_ENS_SUBDOMAIN_TEST;
  ...
  if (chainId !== 31337) {
    ...
    setTimeout(async () => {
      console.log("Verify on Lineascan");
      try {
