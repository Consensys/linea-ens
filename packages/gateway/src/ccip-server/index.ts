...
import { ethers, BytesLike, ParamType, isAddress, isBytesLike } from "ethers";
import {
  Fragment,
  FunctionFragment,
  Interface,
...
export class Server {
  ...
  async call(call: RPCCall): Promise<RPCResponse> {
    ...
    const args = ethers.AbiCoder.defaultAbiCoder().decode(
      handler.type.inputs as unknown as ParamType[],
        ...
        data: handler.type.outputs
          ? hexlify(
              ethers.AbiCoder.defaultAbiCoder().encode(
                handler.type.outputs as unknown as ParamType[],
                result
              )
            )
          : "0x",
